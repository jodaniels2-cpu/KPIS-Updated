...
st.set_page_config(page_title="Modality Lewisham — KPI Dashboard (v9d6: activity type by hour + Klinik units)", page_icon="🏥", layout="wide", initial_sidebar_state="expanded")
...
<div class=\"ml-header\">
  ...
  <div>
    ...
    <p class=\"ml-sub\">Klinik • Docman • Calls — now with activity type by hour in the combined table</p>
  </div>
</div>
""", unsafe_allow_html=True,
)
...
# Combined activity by type
st.markdown("---")
st.markdown("## Combined hourly table — by activity type")
people_all = sorted(pd.concat([klinik_all["person"], docman_all["person"], calls_all["person"]], ignore_index=True).dropna().unique().tolist())
if not people_all:
    st.info("No staff found in the selected date range.")
else:
    default_sel = []
    for key in ["klin_picker","doc_picker","call_picker"]:
        val = st.session_state.get(key)
        if val and val not in default_sel: default_sel.append(val)
    if not default_sel and people_all: default_sel = people_all[:1]

    picks = st.multiselect("Staff to include", options=people_all, default=default_sel,
    ...
    if not picks:
        ...
        if combined_all.empty:
            ...
            day_label_map = {d: pd.Timestamp(d).strftime("%A %d %B") for d in valid_days}
            pick_day = st.selectbox("Choose day", [day_label_map[d] for d in valid_days], key="combined_day_v9d6")
            ...
            if sel.empty:
                ...
                st.dataframe(out[["Hour","Klinik","Docman","Calls_Group","Calls_Callback","Total"]], use_container_width=True, height=380)
                fig = go.Figure()
